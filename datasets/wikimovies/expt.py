...
def configure_from_command_line():
  ...
  optlist,_ = getopt.getopt(sys.argv[1:], 'x', argspec)
...
def run_main():
  ...
  dbspec = 'train-%d.db|train-%d.cfacts' % (c.num,c.num) if c.action=='train' else c.model
  ...
  if c.action=='test':
    ...
    test_data = tlog.load_small_dataset('test-%d.exam' % c.num)
    ...
    train_data = tlog.load_big_dataset('train-%d.exam' % c.num)
